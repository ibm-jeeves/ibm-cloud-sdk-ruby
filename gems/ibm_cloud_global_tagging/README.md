...
So, you can add filters to those attributes when [searching for resources](https://cloud.ibm.com/docs/apis/search).
...
### Getting tags attached to a resource

A common use case is retrieving the tags attached to a resource. To learn how to do this, see [Searching for tags by using the API](https://cloud.ibm.com/docs/account?topic=account-tag&interface=api#search-tags-api-link).
...
Obtaining an IAM token for an authenticated user or service ID is described in the [IAM Identity Services API](https://cloud.ibm.com/docs/apis/iam-identity-token-api) documentation.
...
For more information about how to track tagging activity, see [Auditing events for account management](https://cloud.ibm.com/docs/atracker?topic=atracker-at_events_acc_mgt).
...
After tagging resources, you can search for them by the tag name or resource groups or building even more complex search strings. For more information about advanced search capabilities that use the Lucene query syntax, see the [Global Search API](https://cloud.ibm.com/docs/apis/search).
